    ...
    return (
        <View style={{ margin: 5 }}>
            <ScrollView horizontal={true}>
                ...
                <Image style={{ width: 44, height: 44, borderRadius: 22, borderWidth: 2, backgroundColor: "#ccc", borderColor: value?.brokerId === "0" ? "#975bd9" : "transparent" }} source={{ uri: "https://ik.imagekit.io/lajz2ta7n/Brokers/ALL.png"}} />
                ...
                data?.brokerList?.map((item, index) => {
                    return (
                        <TouchableOpacity key={index} onPress={() => setDeafult(item)} style={{ backgroundColor: "transparent", margin: 5, alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                            <Image style={{ width: 44, height: 44, borderRadius: 22, borderWidth: 2, backgroundColor: "#ccc", borderColor: value?.brokerId === item?.id ? "#975bd9" : "transparent" }} source={{ uri: item?.iconLink }} />
